defmodule FailureModel do
    ...
    def start(num_of_nodes, num_of_requests, num_of_fail) do
        # Spawn master process       
        mpid = spawn_link(FailureModel, :master, [num_of_nodes, num_of_requests, 0, 0, num_of_fail])
        ...
        receive do
            ...
            {:EXIT, pid, msg} ->
                IO.inspect "Exit in FailureModel start from #{inspect pid}"
                IO.inspect msg             
        end
    end
    
    def master(num_of_nodes, num_of_requests, total_hopcount, num_routed, num_of_fail) do
        receive do
            {:construct_topology} ->
                node_digits = 8 
                node_index = 8
                ...
                send self, {:initiate_routing, node_map, node_list, 0, 0}
                #send self, {:initiate_failure, node_map, node_list, 0}

            {:initiate_routing, node_map, node_list, count, count2} ->
                if count < num_of_nodes do 
                    ...
                    if count2 < round(num_of_fail) do
                        send Map.get(node_map, node_id), {:fail_node}
                        count2 = count2 + 1
                    end
                    send self, {:initiate_routing, node_map, node_list, count, count2}
                end 

            #{:initiate_failure, node_map, node_list, count} ->
            #    if count < num_of_fail do 
            #        rand = :rand.uniform(num_of_nodes)-1
            #        node_id = Map.get(node_list, rand)
            #        IO.puts "Send fail_node to #{inspect Map.get(node_map, node_id)} with node_id #{inspect node_id}" 
            #        send Map.get(node_map, node_id), {:fail_node}
            #        count = count + 1
            #        IO.puts "Send initiate_failure to itself with pid #{inspect self} with count #{inspect count}"
            #        send self, {:initiate_failure, node_map, node_list, count}
            #    end 
            ...
            {:terminate, hop_count} ->
                ...
                if(num_routed >= (num_of_nodes - num_of_fail) * num_of_requests) do
        ...
        master(num_of_nodes, num_of_requests, total_hopcount, num_routed, num_of_fail)
    end
    ...
    def node(node_id, num_of_nodes, num_of_requests, node_digits, node_index, b, node_map, node_list, left_leaf_set, right_leaf_set, routing_table, neighbor_set, count, timer, hop_count) do
        receive do
            ...
            {:send_route_node, key, msg, hop_received, key_int} ->                  
                ...
                #IO.puts "routing_table is #{inspect routing_table}"
            ...
            {:fail_node} ->
                if Map.size(left_leaf_set) > 0 do
                   size = Map.size(right_leaf_set)
                   update_right_leaf_set(node_map, right_leaf_set, size, 0, node_id, Map.get(left_leaf_set, Map.size(left_leaf_set)-1))
                else
                    size = Map.size(right_leaf_set)
                    update_right_leaf_set(node_map, right_leaf_set, size, 0, node_id, '-1')
                end
                
                if Map.size(right_leaf_set) > 0 do
                    size = Map.size(left_leaf_set)
                    update_left_leaf_set(node_map, left_leaf_set, size, 0, node_id, Map.get(right_leaf_set, 0) )
                else 
                    size = Map.size(right_leaf_set)
                    update_left_leaf_set(node_map, left_leaf_set, size, 0, node_id, '-1')
                end
                
                for i <- 0..Map.size(node_list) - 1 do
                    node_notify = Map.get(node_list, i)
                    send Map.get(node_map, node_notify), {:remove_from_rt, node_id}
                end

                #IO.puts "killllllllllllllllllllllllllllllllllll node_id #{inspect node_id} with pid #{inspect self}"
                Process.exit(self, :kill)
                
                {:remove_from_rt, id} ->
                    matched_index = -1
                    returned_index = findPrefix(id, node_id, 0)
                    if returned_index > 0 do
                        matched_index = returned_index + 1
                    else
                        matched_index = 0
                    end    
                    col = String.to_integer(List.to_string([Enum.at(id, matched_index)]))
                    if Matrix.elem(routing_table, matched_index, col) == id and matched_index >= 0 do   
                        routing_table = Matrix.set(routing_table, matched_index, col, '-1') 
                        for i <- 0..b-1 do
                            if Matrix.elem(routing_table, matched_index, i) != node_id and Matrix.elem(routing_table, matched_index, i) != id and Matrix.elem(routing_table, matched_index, i) != '-1' and Matrix.elem(routing_table, matched_index, i) != '' do 
                                node = Map.get(node_map, Matrix.elem(routing_table, matched_index, i))
                                send node, {:update_rt, matched_index, col, self}
                            end
                        end
                    end

                {:update_rt, matched_index, col, pid} -> 
                    if Matrix.elem(routing_table, matched_index, col) != '-1' and Matrix.elem(routing_table, matched_index, col) != '' do
                        send pid, {:recover_rt, matched_index, col, Matrix.elem(routing_table, matched_index, col)}
                    end

                {:recover_rt, matched_index, col, new_node} ->
                    #IO.puts "tttttttttttttttttttttttttttttttttttttttttttt for node_id #{inspect node_id} with new_node #{inspect new_node}"
                    if Matrix.elem(routing_table, matched_index, col) == '-1' do
                        Matrix.set(routing_table, matched_index, col, new_node)
                    end
                
                {:update_left_leaf, node_id1, node_id2} ->
                    #IO.puts "lllllllllllllllllllllllllllllllllllllllllllll for node_id1 #{inspect node_id1} and node_id2 #{inspect node_id2}"
                    for i <- 0..Map.size(right_leaf_set) - 1 do
                        if Map.get(right_leaf_set, i) == node_id1 and node_id2 != '-1' do
                            right_leaf_set = Map.put(right_leaf_set, i, node_id2)
                        else 
                            if node_id2 == '-1' do
                                Map.delete(right_leaf_set, i)
                            end
                        end
                    end

                {:update_right_leaf, node_id1, node_id2} ->
                    #IO.puts "rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr for node_id1 #{inspect node_id1} and node_id2 #{inspect node_id2}"
                    for i <- 0..Map.size(left_leaf_set) - 1 do
                        if Map.get(left_leaf_set, i) == node_id1 and node_id2 != '-1' do
                            left_leaf_set = Map.put(left_leaf_set, i, node_id2)
                        else 
                            if node_id2 == '-1' do
                                Map.delete(left_leaf_set, i)
                            end
                        end
                    end
    ...
    defp update_left_leaf_set(node_map, left_leaf_set, size, index, node_id1, node_id2) when index >= size do
    end
    defp update_left_leaf_set(node_map, left_leaf_set, size, index, node_id1, node_id2) do
        node = Map.get(left_leaf_set, index)
        send Map.get(node_map, node), {:update_left_leaf, node_id1, node_id2}
        index = index + 1
        update_left_leaf_set(node_map, left_leaf_set, size, index, node_id1, node_id2)
    end

    defp update_right_leaf_set(node_map, right_leaf_set, size, index, node_id1, node_id2) when index >= size do
    end
    defp update_right_leaf_set(node_map, right_leaf_set, size, index, node_id1, node_id2) do
        node = Map.get(right_leaf_set, index)
        send Map.get(node_map, node), {:update_right_leaf, node_id1, node_id2}
        index = index + 1
        update_right_leaf_set(node_map, right_leaf_set, size, index, node_id1, node_id2)
    end
    ...
    defp construct_topology(num_of_nodes, num_of_requests, c, node_map, node_list1, node_digits, node_index, b) when c < num_of_nodes do
        nid = spawn(FailureModel, :node, [0, num_of_nodes, num_of_requests, node_digits, node_index, b, %{}, %{}, %{}, %{}, [[]], %{}, 0, :timer, 0])
        Process.monitor(nid)
    ...
    def build_maps(num_of_nodes, num_of_requests, node_list, node_map, sorted_node_list, node_digits, node_index, b) do
        ...
        for count <- 0..num_of_nodes-1 do
            ...
            {left_leaf_set, right_leaf_set} = construct_leaf_set(sorted_node_list, left_idx, right_idx, left_count, right_count)
    ...
    defp construct_leaf_set(sorted_node_list, left_idx, right_idx, left_count, right_count) do
        #IO.puts "leaffffffffffffffffffffffffffffffffffffffffffff"
        left_leaf_set = construct_left_set(%{}, sorted_node_list, left_idx, right_idx, left_count, right_count, 0)
        right_leaf_set = construct_right_set(%{}, sorted_node_list, left_idx, right_idx, left_count, right_count, 0)
        #IO.puts "leaf_sets are #{inspect left_leaf_set} and #{inspect right_leaf_set}"
        {left_leaf_set, right_leaf_set}
    end

    defp construct_left_set(left_leaf_set, sorted_node_list, left_idx, right_idx, left_count, right_count, index) when left_count <= 0 do
        left_leaf_set
    end 
    defp construct_left_set(left_leaf_set, sorted_node_list, left_idx, right_idx, left_count, right_count, index) when left_count > 0 do
       #IO.puts "In construct_leaf_set with left_idx #{inspect left_idx} and left_count #{inspect left_count}"
       left_leaf_set = Map.put(left_leaf_set, index, Map.get(sorted_node_list, left_idx))
       index = index + 1
       left_count = left_count - 1
       left_idx = left_idx + 1
       construct_left_set(left_leaf_set, sorted_node_list, left_idx, right_idx, left_count, right_count, index)
    end

    defp construct_right_set(right_leaf_set, sorted_node_list, left_idx, right_idx, left_count, right_count, index) when right_count <=0 do
        right_leaf_set
    end 
    defp construct_right_set(right_leaf_set, sorted_node_list, left_idx, right_idx, left_count, right_count, index) when right_count > 0 do
       #IO.puts "In construct_right_set with right_idx #{inspect right_idx} with right_count #{inspect right_count}"
       right_leaf_set = Map.put(right_leaf_set, index, Map.get(sorted_node_list, right_idx))
       index = index + 1
       right_count = right_count - 1
       right_idx = right_idx + 1
       construct_right_set(right_leaf_set, sorted_node_list, left_idx, right_idx, left_count, right_count, index) 
    end
